...
# Permutation Choice 1 (PC-1) - Key generation
...
# Permutation Choice 2 (PC-2) - Key generation
PC2 = [14, 17, 11, 24, 1, 5,
       3, 28, 15, 6, 21, 10,
       23, 19, 12, 4, 26, 8,
       16, 7, 27, 20, 13, 2,
...
def permute(block, table):
    """
    Permutes the block according to the given table.
    """
...
def hex_to_binary(hex_str):
    ...
    """
    return ''.join(format(int(h, 16), '04b') for h in hex_str)
...
def left_shift(key, n):
    """
    Performs a circular left shift on the key by n positions.
    """
    return key[n:] + key[:n]

def pad_text(text):
    """
    Pads the text to ensure its length is a multiple of 8 bytes (64 bits).
    Uses PKCS#7 padding.
    """
